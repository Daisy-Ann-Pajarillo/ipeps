...
const baseSchema = yup.object().shape({
  ...
  barangay: yup.string().when("country", {
    is: "Philippines",
...
const jobseekerSchema = yup.object().shape({
  ...
  looking_for_a_work_since: yup.string().when("looking_for_a_work", {
    is: "YES",
    then: (schema) => schema.required('Please specify since when you are looking for work'),
    otherwise: (schema) => schema.notRequired(),
  }),
  ...
  ofw_country: yup.string().when("an_ofw", {
    is: "YES",
    then: (schema) => schema.required('Please specify the country where you are working as an OFW'),
    otherwise: (schema) => schema.notRequired(),
  }),
  ...
  former_ofw_country: yup.string().when("former_ofw", {
    is: "YES",
    then: (schema) => schema.required('Please specify the country where you previously worked as an OFW'),
    otherwise: (schema) => schema.notRequired(),
  }),
...
const employerSchema = yup.object().shape({
  ...
  employer_contact_number: yup.string().required('Employer Contact Number is required'),
...
const PersonalInfo = ({ activeStep, steps, handleBack, handleNext, isValid, setIsValid, user_type }) => {
  const defaultValues = {
    first_name: '',
    last_name: '',
    middle_name: '',
  ...
  const [selectedPermanentProvince, setSelectedPermanentProvince] = useState(null);
  const [selectedPermanentMunicipality, setSelectedPermanentMunicipality] = useState(null);
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} sm={6}>
          <TextField fullWidth label="Middle Name" {...register('middle_name')} />
        </Grid>
        <Grid item xs={12} sm={6}>
          <TextField
            fullWidth
        ...
        <Grid item xs={12} sm={6}>
          <FormControl fullWidth required>
            ...
            <p className="text-red-500 text-sm">{errors.civil_status?.message}</p>
          </FormControl>
        </Grid>
          ...
          <Grid item xs={12} sx={{ display: 'flex', justifyItems: 'center' }}>
            <label>
              ...
              <Switch
                {...register('permanent')}
                checked={isPermanent}
                onChange={() => { setIsPermanent(val => !val) }}
                color="primary"
        ...
        {!isPermanent && (
          ...
          <>
            ...
            <Grid item xs={12} sm={6}>
              <Autocomplete
                options={selectedPermanentProvinceData ? selectedPermanentProvinceData.municipalities : []}
                getOptionLabel={(option) => option}
        ...
        {(user_type === 'JOBSEEKER' || user_type === 'STUDENT') && (
          <>
            ...
            <ApplicationDivider />
            {/*  ofw */}
            <Grid item xs={12} className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
              <Grid item xs={12} >
                <Autocomplete
                  options={["YES", "NO"]}
                  getOptionLabel={(option) => option}
