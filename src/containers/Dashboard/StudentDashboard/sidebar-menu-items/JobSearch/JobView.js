...
const JobView = ({ job, onClose, isMobile = false, setIsCollapsed }) => {
  ...
  // Auto-collapse the sidebar when viewing job details on any screen size
  useEffect(() => {
    if (job && setIsCollapsed) {
      setIsCollapsed(true);
      return () => {
  ...
  // Auto-collapse sidebar on desktop when showing job details
  useEffect(() => {
    if (!isMobile && job && setIsCollapsed) {
      setIsCollapsed(true);
      return () => {
        // Restore sidebar when unmounting
        setIsCollapsed(false);
      };
    }
  }, [job, isMobile, setIsCollapsed]);
  ...
  return (    <div className={`bg-white dark:bg-gray-900 flex flex-col ${isMobile
      ? 'h-[85vh] w-full'
      ...
      <div className={`${isMobile ? 'pt-12' : ''
        } px-2 sm:px-3 md:px-4 py-2 sm:py-3 border-b border-gray-200 dark:border-gray-700 flex-shrink-0`}>
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-0">
          ...
          <Button
            onClick={handleSave}
            disabled={isLoading}
            className={`min-w-[70px] sm:min-w-[90px] text-xs sm:text-sm ${isSaved
      ...
      <div className="px-3 sm:px-4 md:px-6 py-3 sm:py-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 flex-shrink-0">
        <Button
          ...
          className={`h-12 rounded-xl font-semibold text-sm ${isApplied
