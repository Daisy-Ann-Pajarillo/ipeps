...
const Placement = () => {
    ...
    return (
        <Box sx={{ p: 3 }}>
            ...
            >
                {["Placement Report by Country", "City/Municipality Placement of Hired Users", "Placement Hired Users by District"].map((label, index) => (
            ...
            <Grid container spacing={2}>
                    ...
                    <Card sx={{ width: '100%', p: 2 }}>
                        <CardContent>
                            <Typography variant="h6" gutterBottom>
                                {["Job Placement by Country", "Placement by City/Municipality", "Placement by District"][tabIndex]}
                            </Typography>

                            {filteredResults.length > 0 ? (
                                filteredResults.map(([key, value], index) => (
                                    <Box key={index} sx={{ mb: 2 }}>
                                        <Typography variant="body2">{key}: {value} placements</Typography>
                ...
                <Grid item xs={12} md={6}>
                    <Card sx={{ width: '100%', p: 2 }}>
                        <CardContent>
                            <Typography variant="h6" gutterBottom>Placement Summary</Typography>
                            {filteredResults.length > 0 ? (
                                filteredResults.map(([key, value], index) => (
                                    <Typography key={index} variant="body2" gutterBottom>
                                        {key}: {value} placements
                                    </Typography>
                                ))
                            ) : (
                                <Typography variant="body2">No data available.</Typography>
