...
const Trend = () => {
    ...
    useEffect(() => {
        if (Array.isArray(JobSeekerAppliedTheirJob)) {
            const uniqueMunicipalities = [...new Set(JobSeekerAppliedTheirJob.map(job => job.municipality))];
            const uniqueIndustries = [...new Set(JobSeekerAppliedTheirJob.map(job => job.jobTitle))];
    ...
    return (
        <Box sx={{ p: 3 }}>
            ...
            >
                {[
                    "Registration by Sex",
                    "Trend by Educational Attainment",
                    "Trend by Courses",
                    "Trend by Municipality/City",
                    "Real-Time In-Demand Jobs"
                ].map((label, index) => (
                    ...
                    <Card sx={{ width: '100%', p: 2 }}>
                        <CardContent>
                            <Typography variant="h6" gutterBottom>
                                {[
                                    "JobSeeker and Their Applied Jobs",
                                    "Top Skills By Municipality",
                                    "Top Skills by Municipality and Industry",
                                    "Top Skills Overseas",
                                    "Applied JobSeeker by Job Group"
                                ][tabIndex]}
                            </Typography>
                            ...
                            {filteredResults.length > 0 ? (
                                filteredResults.map(([key, value], index) => (
                                    <Box key={index} sx={{ mb: 2 }}>
                                        <Typography variant="body2">{key}: {value} applicants</Typography>
